...
X_test_3M_708 = X_test_3M_708.drop([
    'Store',
    'StoreType',
    'Assortment',
    'CompetitionDistance',
...
def build_model(neurons):
    model = Sequential()
    model.add(Dense(neurons, input_shape=[9], activation='relu'))
    model.add(Dense(1))
...
grid_search = grid_search.fit(X_train_708, y_train_708)
...
history = ffnn_model.fit(X_train_708, y_train_708, epochs=best_parameters['epochs'],
                         batch_size=best_parameters['batch_size'], verbose=0)

# Save the model
ffnn_model.save('../../04_Evaluation/00_Models/ffnn_model_708_gs')

# Show the learning curves
history_df = pd.DataFrame(history.history)
history_df.plot()
path = "../../../../data/rossmann/output/ffnn_learning_curve_708"
plt.savefig(path)
# plt.show()
